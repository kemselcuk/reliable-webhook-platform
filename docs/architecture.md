...
The browser uses the real REST API and does not maintain a second delivery state store. Endpoint state can be changed only through `PATCH /api/webhook-endpoints/{id}/enabled` with `{ "enabled": true|false }`; URL editing, deletion, and secret rotation are intentionally absent. Event submission uses the existing endpoint selection and optional `Idempotency-Key` header. `GET /api/deliveries` provides bounded pagination and an enum status filter; `GET /api/deliveries/{id}` returns operator metadata and ordered immutable attempts without payload or secret material. `POST /api/deliveries/{id}/replay` is shown only for `FAILED`/`DEAD` detail state. `GET /api/system/summary` combines bounded database backlog/status counts with process-lifetime accepted-event and delivery-intent counters; no high-cardinality labels or operational secrets are exposed.
